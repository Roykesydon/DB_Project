...
if($auth->isAuth())
{
    ...
    //check if created
    $check_room_ID = "SELECT `room_ID` FROM `roomQueue` WHERE `room_ID` = ?;";
    ...
    {
        echo json_encode(array("message" => "Unable to create roomQueue. Because the room has created."));
    }
    else if($check_room_stmt->rowCount() == 0)
    {
        echo json_encode(array("message" => "Unable to create roomQueue. Because we don't have the room!"));
    }
    else{
        if(//check not null
            !empty($data["room_ID"])
